...
/// Native implementation of [GoogleAccountService] for Android, iOS, macOS,
/// Windows, and Linux.
///
/// Unlike the web version, sign-in here uses
/// [GoogleSignIn.attemptLightweightAuthentication] (silent/cached sign-in) and
/// the standard OAuth flow — no GIS button is required.
///
/// The [buildWebGoogleAction] and [buildWebWarningBanner] methods are kept for
/// API compatibility but render native-appropriate widgets instead of
/// web-specific ones.
class GoogleAccountService {
  ...
  /// Attempts a lightweight (cached/silent) sign-in on native platforms.
  ///
  /// If no cached credentials are found the user will need to sign in
  /// interactively via [GoogleSignIn.authenticate] (trigger from a button).
  ...
  /// On native platforms, renders a standard sign-in / authorize button.
  ///
  /// The GIS web button is not available here; a plain Flutter button is used
  /// instead. The [onPressed] callback should trigger the interactive sign-in
  ...
  Widget buildWebGoogleAction(Function() onPressed) {
    ...
    // Not signed in — show a standard sign-in button for native platforms.
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 8.0),
      child: SizedBox(
        height: 36,
  ...
  Widget? buildWebWarningBanner() {
    ...
    if (!isSignedIn || !isAuthenticated) {
      message =
          'You are not signed in to your Google account. Tap the sign-in button to sync bills from Gmail.';
      icon = Icons.account_circle_outlined;
    } else {
      message =
          'Gmail access has not been authorized. Tap "Authorize Gmail" to allow the app to read your bill emails.';
